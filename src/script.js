...
constructor() {
    ...
    render() {
      ...
      <input class="todo__item-checkbox"
      id="${id}"
      type="checkbox"
      ${idChecked ? 'checked' : ''}
    ...
    filter () {
        const queryFormated = this.state.searchQuery.toLoweCase()
    ...
    onNewTaskFormSubmit = (event) => {
        ...
        const newTodoItemTitle = this.newInputElemnt.value
    ...
    bindEvents() {
        this.newTaskElement.addEventListener('submit', this.onNewTaskFormSubmit)
        this.seacrhTaskFormElement.addEventListener('submit', this.onSearchTaskForm.Submit)
        ...
        this.deleteAllButtonElement.addEventListener('click', this.onClick)
        this.deleteAllButtonElement.addEventListener('click', this.onChange)
    }   
}
new Todo();